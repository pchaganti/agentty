...
Because this runs on the submit thread, it is bounded by a small wall-clock **hedge** (`AGENTTY_RAG_PROACTIVE_BUDGET_MS`, default `250`). The result is *progressive*: on a fast corpus the funnel finishes within the hedge and the grounding rides the same turn's first request. On a large or slow corpus — where the synchronous dense query-embed round-trip would overrun — the submit thread never blocks (Enter can't freeze the UI); the retrieval instead finishes on a background worker and is injected on the *next* turn. So grounding is deferred by at most one turn, never dropped, and the cross-turn dedup FIFO is only committed for blocks that were actually shown. Set the hedge to `0` to always ground a turn late (never touch the submit thread), or raise it to prefer same-turn grounding on a fast index.
