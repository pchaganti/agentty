...
| `AGENTTY_RAG_PROACTIVE_BUDGET_MS` | Wall-clock **hedge** for the proactive pre-turn retrieval on the submit thread. If the funnel (BM25 + dense query-embed + rerank/MMR over the whole index) finishes within the budget, the grounding rides this turn's first request; if it overruns on a large/slow corpus, the submit thread never blocks (the UI can't freeze on Enter) and the retrieval instead completes on a background worker and is injected on the *next* turn — deferred by one turn, never dropped. Default `250`; `0` skips the synchronous hedge entirely (always ground a turn late). Raise it to prefer same-turn grounding on a fast/small corpus. |
