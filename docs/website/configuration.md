...
| `AGENTTY_RAG_GRAPH` | GraphRAG-lite: follow the top hits' outbound markdown links one hop and add the linked documents' lead chunks as supporting material (always ranked below direct hits). **On by default** (deterministic, in-memory); `=0` disables. |
